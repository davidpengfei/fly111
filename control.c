	...
	switch(status)
	{
		case WAITING_1:
		  if(Command.FlightMode != LOCK)  //检测解锁
			...
			break;
		case WAITING_2: //
			if(Command.FlightMode == GPS_POSITION)//如果进入定高飞行模式
			{
				status = WAITING_3;
			}
			break;
		case WAITING_3:
			pidRest((PidObject*)&pidData.GPS_Latitude_rate,4);  //复位上次遗留下来的定高PID数据
		...
		case PROCESS_31:
			{
			  ...
			  if(GPS.satellite_num<10||GPS.Quality>3.0f || Earth_Position.Filter_Defeated_Flag)//定位卫星小于10个或者水平精度因子小于3不可用  //或者融合失败
				{
					break;
				}
				const uint16_t DEADBAND=100;

				if((REMOTE_PITCH>(1500+DEADBAND))||(REMOTE_PITCH<(1500-DEADBAND))||(REMOTE_ROLL>(1500+DEADBAND))||(REMOTE_ROLL<(1500-DEADBAND))) //拨动遥杆只进行水平速度控制
				{
						if(REMOTE_PITCH>(1500+DEADBAND))
						{
							pidData.GPS_Longitude_rate.desired = (REMOTE_PITCH -(1500+DEADBAND))*0.4f;//最大速度500*0.3 = 150cm/s
						}
						else if(REMOTE_PITCH<(1500-DEADBAND))
						{
							pidData.GPS_Longitude_rate.desired = (REMOTE_PITCH -(1500-DEADBAND))*0.4f;
						}
						if(REMOTE_ROLL>(1500+DEADBAND))
						{
							pidData.GPS_Latitude_rate.desired = -(REMOTE_ROLL -(1500+DEADBAND))*0.4f;
						}
				...
				else
				{
						...
						{
							pidData.GPS_Latitude_position.desired = Earth_Position.Latitude_N_S_Distance;//记录位置
							pidData.GPS_Longitude_position.desired = Earth_Position.Longitude_W_E_Distance;//记录位置	
						...
						{
							position_delay = 0;
							pidData.GPS_Latitude_position.measured = Earth_Position.Latitude_N_S_Distance;//获取位置信息
							pidData.GPS_Longitude_position.measured = Earth_Position.Longitude_W_E_Distance;//获取位置信息		
						
							float Longitude_Error = pidData.GPS_Longitude_position.desired - pidData.GPS_Longitude_position.measured;	
							float Latitude_Error = pidData.GPS_Latitude_position.desired - pidData.GPS_Latitude_position.measured;					
							float Pitch_Error_Body  = 	-Longitude_Error*IMU.Sin_Yaw+Latitude_Error*IMU.Cos_Yaw;	//从地理坐标转为机体坐标
							float	Roll_Error_Body = Longitude_Error*IMU.Cos_Yaw+Latitude_Error*IMU.Sin_Yaw;

							Pitch_Error_Body =LIMIT(Pitch_Error_Body,-pidData.GPS_Longitude_position.Err_Max, pidData.GPS_Longitude_position.Err_Max);//位置偏差限幅，单位cm
							Roll_Error_Body = LIMIT(Roll_Error_Body,-pidData.GPS_Latitude_position.Err_Max,pidData.GPS_Latitude_position.Err_Max);//位置偏差限幅，单位cm

							pidData.GPS_Longitude_rate.desired =  Pitch_Error_Body * pidData.GPS_Longitude_position.Kp ; 
							pidData.GPS_Latitude_rate.desired =  Roll_Error_Body * pidData.GPS_Latitude_position.Kp ;			
						}
				}
				    //地理坐标系转化为沿机体Pitch、Roll方向水平速度控制
					pidData.GPS_Longitude_rate.measured = -Earth_Position.Longitude_W_E_Speed * IMU.Sin_Yaw+Earth_Position.Latitude_N_S_Speed*IMU.Cos_Yaw;
					pidData.GPS_Latitude_rate.measured =Earth_Position.Longitude_W_E_Speed*IMU.Cos_Yaw+Earth_Position.Latitude_N_S_Speed*IMU.Sin_Yaw;	
				
				
					PID_Control_Err_LPF(&pidData.GPS_Longitude_rate);//调用PID处理函数来处理
					PID_Control_Err_LPF(&pidData.GPS_Latitude_rate);//调用PID处理函数来处理
			}	
			break;
		case EXIT_255: //退出定高
			pidRest((PidObject*)&pidData.GPS_Latitude_rate,4);  //复位上次遗留下来的定高PID数据
				...
				//-------------------------高度环-------------------------------
					...
					else 
					{
						...
						if(high_delay>1)
						{
							  high_delay=0;
								//------------高度环控制-----------
								pidData.HeightHigh.measured =  Height.High; //获取高度情况
					...
					Throttle_out = High_Hold_Throttle + pidData.HeightAccel.Control_OutPut;
...
void AttitudePidControl(void)
{
	...
	{		
		...
		case PROCESS_31: //正式进入控制
			...
			pidData.Yaw.measured = yaw_control;
			...
			if(REMOTE_YAW < 1700 && REMOTE_YAW > 1300)//偏航杆置于中位
			 {					
