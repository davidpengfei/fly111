...
static uint8_t PositionX_PID[18];
static uint8_t PositionY_PID[18];
//接收到上位机的数据种类标志位
static struct{
...
#define ANTO_POSITION_X_PID  ANTO_PID4
#define ANTO_POSITION_Y_PID  ANTO_PID5
/***********************************************************************
 * 
 * @param[in] 
 * @param[out] 
...
void ANO_Recive(int8_t *pt)                   //接收到上位机的数据
{
	switch(pt[2])
	{
		...
		case ANTO_HEIGHT_PID:                     //这组的PID是给高度环用的
			...
			break;
		case ANTO_POSITION_X_PID://接收到左右位置控制的PID
			memcpy(PositionX_PID,&pt[4],18);
			ANTO_Recived_flag.PID4 = 1;
			break;
		case ANTO_POSITION_Y_PID: //接收到前后位置控制的PID
			memcpy(PositionY_PID,&pt[4],18);
...
void ANTO_Send(const enum ANTO_SEND FUNCTION) 
{
	...
	switch(FUNCTION)
	{
		case ANTO_RATE_PID:      //发送PID1到上位机
				 pidX = &pidData.RateY;   //指定发送数据为角速度内环 //roll对应角速度Y
				 pidY = &pidData.RateX;
				 pidZ = &pidData.RateZ;
         goto send_pid;		
		...
		case ANTO_HEIGHT_PID:     //发送PID3到上位机			
				 ...
				 goto send_pid;							
		case ANTO_POSITION_X_PID:	  //PID4
				 pidX = &pidData.GPS_Latitude_rate; //左右位置控制
				 pidY = &pidData.GPS_Latitude_position;  //	
				 pidZ = NULL;//&pidData.GPS_Latitude_Acce; //加速度环PID		//GPS定点暂时只用到双闭环
				 goto send_pid;							
		case ANTO_POSITION_Y_PID:	         //PID5
				 pidX = &pidData.GPS_Longitude_rate; //前后位置控制
				 pidY = &pidData.GPS_Longitude_position;  //	
				 pidZ = NULL;//&pidData.GPS_Longitude_Acce; //加速度环PID		//GPS定点暂时只用到双闭环
				 goto send_pid;							
    case ANTO_PID6:
send_pid:
			if(pidX!=NULL)
		...
		case ANTO_RCDATA: //send RC data
			...
			Anto[6] = Device.Remote.AUX1;
			Anto[7] = Device.Remote.AUX2;
			Anto[8] = Device.Remote.AUX3;
			Anto[9] = Device.Remote.AUX4;
			Anto[10] = 0; 
			Anto[11] = 0; 
			len = 20;
			break;
...
//			memcpy(&Anto[2],(int8_t*)&Device.MPU6050,sizeof(Device.MPU6050));//最原始的数据
			Anto[2] = Attitude.accX_origion;//六面矫正后的数据
			Anto[3] = Attitude.accY_origion;
			Anto[4] = Attitude.accZ_origion;
			Anto[5] = Device.MPU6050.gyroX;
			Anto[6] = Device.MPU6050.gyroY;
			Anto[7] = Device.MPU6050.gyroZ;  
			memcpy(&Anto[8],(int8_t*)&Device.AK8975,sizeof(Device.AK8975));
			len = 18;
			break;
		case ANTO_SENSER2:
			{
				long _temp = (long)FlightData.High.bara_height;
				*(uint16_t*)&Anto[2] = *((uint16_t*)&_temp+1);
				*(uint16_t*)&Anto[3] = *((uint16_t*)&_temp);
				Anto[4] = (uint32_t)FlightData.High.ultra_height;
				len = 6;
			}
			break;
		case ANTO_GPSDATA:
	...
	switch(status)
	{
		...
		case 2:
			...
			{		
					...
					delay_ms(1);
					ANTO_Send(ANTO_POSITION_X_PID);
					delay_ms(1);
					ANTO_Send(ANTO_POSITION_Y_PID);
					delay_ms(1);
			...
			if(*(uint8_t*)&ANTO_Recived_flag & 0x3f) //接收到上位机发来的PID数据
			{
				  ...
				  uint8_t *P;
					...
					{
						 pidX = &pidData.Roll;//横滚
						 pidY = &pidData.Pitch;
						 pidZ = &pidData.Yaw;
						 P = AnglePID;	
						 ANTO_Recived_flag.PID2 = 0;                             
					}
					else if(ANTO_Recived_flag.PID3)//接收定高的PID
					{
					...
					{
						 pidX = &pidData.GPS_Latitude_rate;
						 pidY = &pidData.GPS_Latitude_position;
						 //pidZ = &pidData.GPS_Latitude_Acce;//位置没用到双闭环，只用到量环
					 	pidZ = NULL;
						 P = PositionX_PID;	
					...
					else if(ANTO_Recived_flag.PID5) 
					{
						 pidX = &pidData.GPS_Longitude_rate;//前后位置控制
						 pidY = &pidData.GPS_Longitude_position;
						 //pidZ = &pidData.GPS_Longitude_Acce;//位置没用到双闭环，只用到量环
						pidZ = NULL;
						P = PositionY_PID;	
